...
function FunnelBar({ label, value, max, color }: { label: string; value: number; max: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-4">
      <span className="w-28 shrink-0 text-right text-sm text-slate-400">{label}</span>
      <div className="flex-1">
        <div className="h-8 overflow-hidden rounded-lg bg-slate-800/40">
          <div
            className={`h-full rounded-lg ${color} flex items-center px-3 text-sm font-bold text-white transition-all duration-700`}
            style={{ width: `${width}%` }}
          >
            {value > 0 && value}
          </div>
    ...
    <>
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="mt-1 text-slate-400">Track where your leads come from and how they convert.</p>
        </div>
        ...
        <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-6">
          <h2 className="mb-4 text-lg font-bold text-white">Conversion Funnel (Last 30 Days)</h2>
        ...
        <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-6">
          ...
          {sources && sources.length > 0 ? (
            ...
            <div className="py-8 text-center text-slate-500">
              <BarChart3 className="mx-auto h-8 w-8 mb-2 text-slate-600" />
        ...
        <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-6">
          <h3 className="font-bold text-white">Set Up Your Tracking Pixel</h3>
