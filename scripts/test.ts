...
function startPreviewServer(): ChildProcess {
  ...
  const server = spawn(isWindows ? "npm.cmd" : "npm", ["run", "preview"], {
    cwd: projectRoot,
...
async function main() {
  ...
  if (args.length === 0) {
    console.log("Usage: npm run test -- <test-file.ts> [test-file2.ts ...]");
    console.log("Example: npm run test -- scripts/demo-test.ts");
    console.log("\nThis script:");
